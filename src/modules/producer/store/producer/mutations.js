...
export const addProducerStorage = (state, producer) => {
  localStorage.setItem(
    "producersStorage",
    JSON.stringify([...state.producersStorage, producer.value])
  );
...
export const addProductionStorage = (state, production) => {
  localStorage.setItem(
    "productionsStorage",
    JSON.stringify([...state.productionsStorage, production.value])
  );
  state.productionsStorage = JSON.parse(
    localStorage.getItem("productionsStorage")
  );
};
...
export const setProducerPicture = (state, production) => {
  const idx = state.productions.map((p) => p.id).indexOf(production.id);
  state.productions[idx] = production;
  state.production = production;
};
...
export const setProducerStorage = (state, producers) => {
  ...
  state.allProducerStorage = JSON.parse(
    localStorage.getItem("allProducerStorage")
  );
};
